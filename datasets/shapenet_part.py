...
def load_data_partseg(partition):
    ...
    BASE_DIR = ''
    DATA_DIR = os.path.join(BASE_DIR, 'data') #这个就是./data
    all_data = []
    all_label = []
    all_seg = []
    if partition == 'trainval': #这个就是底下全部train开头的6个h5+val开头的1个h5 h5二进制文件看不到内容
    ...
    else: #否则partition只能选择train val test （test开头有2个h5
        file = glob.glob(os.path.join(DATA_DIR, 'shapenet_part_seg_hdf5_data', '*%s*.h5'%partition))
    for h5_name in file: #通过h5py.File(name,'r+)读取h5文件内容
        f = h5py.File(h5_name, 'r+')
        data = f['data'][:].astype('float32') #shape:(2048,2048,3) type:ndarray 这2048个同属于一个大类，但可能属于不同小类，每个点云xyz三维
        label = f['label'][:].astype('int64') #shape(2048,1)
        seg = f['pid'][:].astype('int64') #shape(2048,2048)
        f.close()
