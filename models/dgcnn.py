...
class DGCNN(nn.Module):
    def __init__(self, args, cls=-1):
        ...
        self.args = args
        self.k = args.k
        self.device=torch.device(f"cuda:{args.cudaNo}" if args.cuda else "cpu")
...
class DGCNN_update(nn.Module):
    def __init__(self, args, cls=-1):
        super(DGCNN_update, self).__init__()
        self.args = args
        self.k = args.k
        self.device=torch.device(f"cuda:{args.cudaNo}" if args.cuda else "cpu")
        self.bn1 = nn.BatchNorm2d(64)
...
class PntHead(nn.Module):
    def __init__(self, args, outdim=256):
        super(PntHead, self).__init__()
        self.inv_head = nn.Sequential(
            nn.Linear(args.emb_dims * 2, args.emb_dims),
            nn.BatchNorm1d(args.emb_dims),
            nn.ReLU(inplace=True),
            nn.Linear(args.emb_dims, outdim)
        )

    def forward(self, feat):
        return self.inv_head(feat)


class ResNet_update(nn.Module):
    def __init__(self, model, feat_dim=2048):
        super(ResNet_update, self).__init__()
        self.resnet = model
        self.resnet.fc = nn.Identity()
...
class ImgHead(nn.Module):
    def __init__(self, feat_dim=2048, outdim=256):
        super(ImgHead, self).__init__()
        self.inv_head = nn.Sequential(
            nn.Linear(feat_dim, 512, bias=False),
            nn.BatchNorm1d(512),
            nn.ReLU(inplace=True),
            nn.Linear(512, outdim, bias=False)
        )
